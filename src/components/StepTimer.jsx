...
const StepTimer = ({ duration, onComplete }) => {
    ...
    return (
        <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md p-3 rounded-xl border border-white/20 w-fit mt-4 animate-fade-in">
            <div className="flex items-center gap-2 text-2xl font-mono font-bold text-white">
                <AlarmClock size={24} className={isActive ? 'animate-pulse text-primary' : ''} />
            ...
            <div className="flex gap-2">
                <button
                    onClick={toggleTimer}
                    className={`p-2 rounded-full transition-colors ${isActive ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-green-500 hover:bg-green-600'} text-white`}
                >
